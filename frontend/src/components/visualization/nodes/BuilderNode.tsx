...
export default function BuilderNode({ data }: NodeProps<BuilderNodeData>) {
  const borderColor = data.locked ? '#FFD600' : '#505070'
  const bgColor = data.locked ? 'rgba(255,214,0,0.07)' : '#1A1A25'
  ...
  return (
    ...
    >
      ...
      <div style={{ textAlign: 'center' }}>
        <div style={{ fontSize: 11, fontWeight: 500, color: data.locked ? '#FFD600' : '#A0A0C0', letterSpacing: '0.05em' }}>
          {data.label}
        </div>
        ...
        {data.locked && (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4, marginTop: 4 }}>
            ...
            <span style={{ fontSize: 9, color: '#FFD600', letterSpacing: '0.2em', textTransform: 'uppercase' }}>XT Locked</span>
          </div>
        )}
      </div>
    </div>
